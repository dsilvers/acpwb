...
| `drain_archive_queue` | `--batch`, `--max-batches` | Pops ArchiveVisit records from the Redis queue (`acpwb:archive_queue`) and bulk-inserts them into PostgreSQL. Run by cron every minute. Default: 500 records/batch, max 200 batches per run. |
...
**ArchiveVisit write path:** `archive_trap()` pushes visit data to the Redis list `acpwb:archive_queue` (fast RPUSH) instead of writing to PostgreSQL synchronously. `drain_archive_queue` cron pops and bulk-inserts into the DB. Falls back to direct DB write if Redis is unavailable. No queue depth cap.
