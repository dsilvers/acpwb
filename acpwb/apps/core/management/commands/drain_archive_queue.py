...
from apps.core.crawler_queue import (
    archive_queue_length, finalize_batch, pop_archive_visits, recover_archive_visits,
)
...
class Command(BaseCommand):
    ...
    def _drop_already_inserted(self, items):
        ...
        on here (see ArchiveVisit.idempotency_key — TimescaleDB hypertables
        ...
        if not keys:
            return items
        existing = {
            str(k) for k in ArchiveVisit.objects.filter(
    ...
    def _process_batch(self, items, batch_key, is_recovery=False):
        """
        Insert `items` into the DB, then finalize_batch() the corresponding
        Redis processing key. Returns the number of records inserted.

        finalize_batch() only runs once this has done everything it's going
        to for this batch — if bulk_create raises, the key is left in place
        for recover_archive_visits() to retry next run.
        """
    ...
    def _drain(self, options):
        ...
        recovered = 0
        for items, batch_key in recover_archive_visits():
        ...
        while True:
            if max_seconds and time.monotonic() - started >= max_seconds:
                break
            items, batch_key = pop_archive_visits(batch_size)
